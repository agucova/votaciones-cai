...
def _staticfiles_urlpatterns(prefix=None, name="static"):
    ...
    return [
        re_path(
            ...
            kwargs={"insecure": not settings.DEBUG, "show_indexes": settings.DEBUG,},
            name=name,
        ),
    ]
...
_app_urlpatterns = [
    ...
    path("accounts", _limit_read_access(index_get_view), name="accounts"),
...
if settings.JAWANNDENN_URL_PREFIX:
    ...
    urlpatterns = [
        ...
        path(
            settings.JAWANNDENN_URL_PREFIX.strip("/") + "/", include(_app_urlpatterns)
        ),
    ]
else:
